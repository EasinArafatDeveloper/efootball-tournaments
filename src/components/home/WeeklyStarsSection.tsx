    ...
    <section className="w-full py-4 sm:py-6">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between pb-3 sm:pb-4">
          <div>
            <h2 className="text-sm sm:text-base md:text-lg font-bold text-[#111111] flex items-center">
              Weekly Stars
            </h2>
            <p className="text-[11px] sm:text-xs text-[#5F6368] mt-0.5">This week&apos;s top performers</p>
          </div>
        ...
        {/* Responsive Container: Horizontal Scroll on Mobile, 3 Cols on Tablet, 6 Cols on Desktop */}
        <div className="flex overflow-x-auto md:grid md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-3.5 no-scrollbar snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0 pb-1">
          {weeklyStars.map((star: WeeklyStarItem) => (
            <Link
              key={star.id}
              href={`/players/${star.playerName.toLowerCase().replace(/\s+/g, "_")}`}
              className="group w-[155px] sm:w-[175px] shrink-0 md:w-auto md:shrink snap-start bg-[#FFFFFF] border border-[#E5E7EB] hover:border-[#111111] rounded-xl p-3 sm:p-3.5 flex flex-col justify-between transition-all shadow-sm hover:shadow-md"
            >
              {/* Category Pill */}
              ...
              <div className="my-2.5 sm:my-3 flex justify-center">
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full overflow-hidden bg-[#F7F8FA] border-2 border-[#E5E7EB] group-hover:border-[#111111] relative transition-all shadow-sm">
                  <Image
                    src={star.avatar}
                    alt={star.playerName}
                    fill
              ...
              <div className="text-center space-y-1">
                ...
                <div className="pt-2 border-t border-[#F7F8FA]">
                  <div className="text-[10px] sm:text-[11px] font-black text-[#111111] tracking-tight bg-[#F7F8FA] rounded-md py-1 px-1.5 border border-[#E5E7EB]">
                    {star.statValue}
                  </div>
                </div>
              </div>
