    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-3">
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-1">
              ...
              <div className="text-2xl font-black text-slate-950 font-mono">{player?.rating || 750}</div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-1">
              ...
              <div className="text-2xl font-black text-emerald-700 font-mono">{formatCurrency(player?.marketValue || 50)}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-500 uppercase">Matches Played</div>
          <div className="text-2xl font-black text-slate-950 font-mono">{player?.stats?.matchesPlayed || 48}</div>
          <div className="text-[11px] text-emerald-700 font-bold">{player?.stats?.winRate || 81.3}% Win Rate</div>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-500 uppercase">Goals Scored</div>
          <div className="text-2xl font-black text-slate-950 font-mono">{player?.stats?.goalsScored || 64}</div>
          <div className="text-[11px] text-slate-700 font-semibold">{player?.stats?.assists || 19} Assists</div>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-500 uppercase">MOTM Awards</div>
          <div className="text-2xl font-black text-amber-600 font-mono">{player?.motmCount || 14}</div>
          <div className="text-[11px] text-slate-500">Accredited Honors</div>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-1">
          <div className="text-xs font-bold text-slate-500 uppercase">Current Club</div>
          <div className="text-sm font-black text-slate-950 truncate">{player?.club?.name || "Dhaka Dominators"}</div>
          <div className="text-[11px] text-slate-700 font-semibold">Active Starting XI</div>
        </div>
      </div>
            ...
            {fixtures.map((f) => (
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Link
                    href={`/matches/${f.id}`}
                    className="px-3.5 py-1.5 rounded-xl bg-black text-white font-bold text-xs hover:bg-zinc-800 transition-colors shadow-sm"
                  >
                    Match Hub
                  </Link>
                </div>
