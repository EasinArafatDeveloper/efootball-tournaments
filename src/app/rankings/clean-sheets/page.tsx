...
export default function CleanSheetsPage() {
  ...
  useEffect(() => {
    async function loadCleanSheets() {
      try {
        const res = await fetch("/api/rankings?type=clean-sheets");
        const json = await res.json();
        if (json.success) setDefenders(json.data);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider bg-slate-50">
                ...
                <th className="py-3.5 px-4 text-right">Clean Sheets</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-xs">
              {defenders.map((p, idx) => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="py-3.5 px-4 text-center font-mono text-emerald-800 font-bold">{p.preferredPosition}</td>
                  <td className="py-3.5 px-4 text-center font-mono text-slate-700">{p.stats?.matchesPlayed || 0}</td>
                  <td className="py-3.5 px-4 text-right font-black text-emerald-800 font-mono text-base">
                    {p.stats?.cleanSheets || 0} Clean Sheets
                  </td>
                </tr>
              ))}
            </tbody>
